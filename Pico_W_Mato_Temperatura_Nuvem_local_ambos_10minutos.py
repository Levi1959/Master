...
import ssd1306
import gc # Para limpar a memória RAM
import dht
import sdcard
...
while True:
    try:
        # Limpa a memória RAM não utilizada antes de iniciar o ciclo
        print(f"RAM Livre ANTES: {gc.mem_free()} bytes")
        gc.collect()
        print(f"RAM Livre DEPOIS: {gc.mem_free()} bytes")
