...
class MQTTClient:
    ...
    def connect(self, clean_session=True):
        ...
        if resp[3] != 0:
            raise MQTTException("CONNACK failed: %d" % resp[3])
        return resp[2] & 1
    ...
    def subscribe(self, topic, qos=0):
        ...
        self.sock.write(pkt)
        self._send_str(topic)
        self.sock.write(bytearray([qos]))
        resp = self.sock.read(5)
        if not (resp[0] == 0x90 and resp[2] == pkt[2] and resp[3] == pkt[3]):
    ...
    def wait_msg(self):
        ...
        if res == b"":
            raise OSError(-1)
        if res == b"\xd0":
            self.sock.read(1)  # ignore pingresp
            return None
        op = res[0]
        if op & 0xf0 != 0x30:
            return op
