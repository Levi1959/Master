...
def atualizar_grafico(i):
    try:
        ...
        if os.path.exists(FILE_SD) and os.path.getsize(FILE_SD) > 50:
            # on_bad_lines='skip' ignora linhas quebradas enviadas pelo MQTT
            df_sd = pd.read_csv(FILE_SD, on_bad_lines='skip')
            
            # Força a conversão para número; se houver texto perdido, vira NaN (Not a Number)
            df_sd['leitura'] = pd.to_numeric(df_sd['leitura'], errors='coerce')
            df_sd['temp'] = pd.to_numeric(df_sd['temp'], errors='coerce')
            df_sd['umid'] = pd.to_numeric(df_sd['umid'], errors='coerce')
            
            # Remove qualquer linha que tenha gerado NaN para não quebrar o gráfico
            df_sd = df_sd.dropna(subset=['leitura', 'temp', 'umid'])
            
            ultimos_sd = df_sd.tail(15)

            # Temp SD (Eixo X explícito usando 'leitura')
            ax_temp_sd.plot(ultimos_sd['leitura'], ultimos_sd['temp'], label='Temp SD (°C)', color='#e31a1c', marker='o')
            ...
            # Umid SD (Eixo X explícito usando 'leitura')
            ax_umid_sd.plot(ultimos_sd['leitura'], ultimos_sd['umid'], label='Umid SD (%)', color='#1f78b4', marker='o')
        ...
        if os.path.exists(FILE_REMOTO) and os.path.getsize(FILE_REMOTO) > 50:
            df_re = pd.read_csv(FILE_REMOTO, on_bad_lines='skip')
            
            df_re['leitura'] = pd.to_numeric(df_re['leitura'], errors='coerce')
            df_re['temp'] = pd.to_numeric(df_re['temp'], errors='coerce')
            df_re['umid'] = pd.to_numeric(df_re['umid'], errors='coerce')
            
            df_re = df_re.dropna(subset=['leitura', 'temp', 'umid'])
            
            ultimos_re = df_re.tail(15)

            # Temp Remoto (Eixo X explícito usando 'leitura')
            ax_temp_re.plot(ultimos_re['leitura'], ultimos_re['temp'], label='Temp Remoto (°C)', color='#ff7f00', marker='s')
            ...
            # Umid Remoto (Eixo X explícito usando 'leitura')
            ax_umid_re.plot(ultimos_re['leitura'], ultimos_re['umid'], label='Umid Remoto (%)', color='#33a02c', marker='s')
        ...
        # Agora o erro real será impresso no terminal, facilitando o diagnóstico
        print(f"Erro na renderização do gráfico: {e}")
...
# cache_frame_data=False evita que o Matplotlib pause a animação reclamando de falta de memória
